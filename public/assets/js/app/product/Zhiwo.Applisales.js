...
Zhiwo.Applisales = {
	init: function() {
		...
		$("input[name='mall_price']").keyup(function(){
			var mall_price = $(this).val();
			var cost_price = $(this).parent().parent().find('.cost_price').html();
			if (mall_price < ~~cost_price) {
				alert('商城价低于成本价,请谨慎填写.');
			}
		});
	...
	bindApplySalesAdd: function() {
		$('.applySalesAdd').click(function () {
			var t = $(this);
			var url = '/product/applysales/add';
			...
			if(cost_price > mall_price || cost_price > group_price){
				if(!confirm("商城价格或团购价格比成本价还低，还确认申销吗？")){
					return false;
				}
			}
	...
	bindApplySalesEdit: function() {
		$('.applySalesEdit').click(function () {
			...
			var data = 'goods_id='+goods_id+'&start_time='+start_time+'&end_time='+end_time+'&group_price='+group_price+'&market_price='+market_price;
			var respHandle = function(data) {
				alert(data);
			}
			$.post(url, data, respHandle);
