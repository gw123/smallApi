...
class  Log{
    ...
    public static function recordTime($pointName='')
    {
        $diffTime = 0;
        if( self::$lastTime == 0 )
        {
            self::$lastTime = self::msectime();
            return 'start record';
        }
        else
        {
            $diffTime =  self::msectime() - self::$lastTime;
        }
       
        $ret= self::send('waring', sprintf("执行时间:%5dms  "."当前断点：".$pointName , $diffTime) ,'runtime');
        // 除去发送http请求的时间
        self::$lastTime = self::msectime();
        return $ret;
    }
