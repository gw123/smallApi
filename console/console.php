...
## 基础服务入口
$httpServerConf = ['port'=>8181, 'bind_addr'=>"0.0.0.0.0"];
#测试进程关系
...
//getPid('entry');
#监听地址 0.0.0.0 是全部地址
$manager = new swoole_http_server($httpServerConf['bind_addr'], $httpServerConf['port']);
...
$manager->set(array(
    ...
    'document_root' => '/data/wwwroot/smallApi/api/public',
...
$manager->on("start", function ($server) {
...
{
    //$sql = "";
    $databases = \All::$app->db->createCommand('show databases')->queryAll();
    var_dump($databases);
}
 ...
 *
 */
$manager->on('WorkerStart', function ($serv, $worker_id) use($manager){
    $str =  "工作进程 初始化框架 开始定时器。。。\n";
    echo $str;
    #Worker 进程启动定时器
    $manager->tick(1000, function() use ($serv , $worker_id) {
        crontab();
    });
});
 ...
 */
$manager->on('WorkerStop', function ($serv, $worker_id){
    $str = "进程结束 销毁内存\n";
    getPid($str);
});
 ...
 *
 */
$manager->on("request", function ($request, $response) {
